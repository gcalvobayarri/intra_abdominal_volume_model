...
plot(seq(min(X, na.rm = T), max(X, na.rm = T), length.out = length(PIA_standard)),
     Predictive_mean_age_men_mean, type = 'l', ylim = c(0, 10), 
     xlab = 'Intra-abdominal pressure', ylab = 'Intra-abdominal volume')

polygon(c(seq(min(X, na.rm = T), max(X, na.rm = T), length.out = length(PIA_standard)), 
          rev(seq(min(X, na.rm = T), max(X, na.rm = T), length.out = length(PIA_standard)))), 
        c(Predictive_mean_age_men_q025, rev(Predictive_mean_age_men_q975)), 
        col = 4, border = NA)

lines(seq(min(X, na.rm = T), max(X, na.rm = T), length.out = length(PIA_standard)), 
      Predictive_mean_age_men_mean)
...
plot(seq(min(X, na.rm = T), max(X, na.rm = T), length.out = length(PIA_standard)), 
     Predictive_mean_age_women_mean, type = 'l', ylim = c(0, 10), 
     xlab = 'Intra-abdominal pressure', ylab = 'Intra-abdominal volume')

polygon(c(seq(min(X, na.rm = T), max(X, na.rm = T), length.out = length(PIA_standard)), 
          rev(seq(min(X, na.rm = T), max(X, na.rm = T), length.out = length(PIA_standard)))), 
        c(Predictive_mean_age_women_q025, rev(Predictive_mean_age_women_q975)), 
        col = 3, border = NA)

lines(seq(min(X, na.rm = T), max(X, na.rm = T), length.out = length(PIA_standard)), 
      Predictive_mean_age_women_mean)
